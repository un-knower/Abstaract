...
//    private static Log LOG = LogFactory.getLog(SemanticSimilarity.class);
    ...
    public static SemanticSimilarity getInstance(){
    	...
    	}else instance = new SemanticSimilarity();
    	return instance;
    }
...
//        System.out.println(Arrays.toString(secondList));
    ...
    public double calculate(String[] firstList, String[] secondList){    	
    	...
    	//System.out.println("wordSim（词形相似度） ==> " + wordSim + ", ordSim（词序相似度） ==> " + ordSim);
    ...
    public String[] segment(String sentence) {
		...
		byte[] spiltResult = CLibrary.Instance.
				NLPIR_ParagraphProcess(sentence, 0).getBytes();
		String spiltResultStr;
		String[] result = null;
		try {
			spiltResultStr = new String(spiltResult, 0,
					spiltResult.length, "utf-8");
			result = spiltResultStr.split(" ");//将题干分词
